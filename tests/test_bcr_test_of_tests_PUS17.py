...
if __name__ == "__main__":
   ...
   LOGGER.info(f"### TEST OF TESTS ###")
   TEST_OF_TESTS = [[Are_You_Alive_Conn, 1], [Report_Verif, 128]]
   seq_count = 0x00
   for test in TEST_OF_TESTS:
      test_id = test[0]
      LOGGER.info(f"# SCENARIO {seq_count}: {test_id} TEST ")
      json_object = json.dumps(test[0])
      #                           SEQCOUNT MESS       APID  T ST
      cmd2bcr = SpacePacketCommand(seq_count, json_object, APID, 17, test[1])
